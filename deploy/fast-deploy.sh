...
# 全程零下载，数分钟完成。
#
# ⚠️  cp 内容跨 docker restart 存活，但 compose up -d 重建容器后会丢失，
#     回退为镜像内容。因此：
#   - 日常代码变更 → 本脚本即可
...
echo "==> [2/3] rsync → $HOST:$REMOTE_DIR（排除 data/.git/node_modules/office-tool）"
rsync -az \
  --exclude .git \
...
echo "==> [3/3] docker cp 灌入 $CONTAINER:/out 并重启"
...
printf 'health: '; curl -s --max-time 8 http://127.0.0.1/api/health; echo
"
echo "==> 部署完成"
