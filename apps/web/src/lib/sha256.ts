/**
 * 纯 JS SHA-256：仅在非安全上下文（HTTP 部署时 crypto.subtle 不存在）作 PoW 回退。
 * 语义与 WebCrypto digest('SHA-256') 一致：输入任意字节，输出 32 字节大端摘要。
 */
...
/** 一次性 SHA-256（输入须小于 512MB，PoW 场景远达不到） */
